...
<head>
    ...
    <title>我的购物车</title>
    <link rel="shortcut icon" type="image/x-icon" href="/static/images/shopping_cart.ico"  media="screen"/>
    ...
    <c:choose>
        <c:when test="${empty cartList}">
            <h3 class="text-success" style="text-align: center;margin-top: 180px">
               <img src="/static/images/null.png" style="width: 80px;height: 80px;"> <a style="font-size: 20px;" class="btn-link" href="${pageContext.request.contextPath}/front/product/searchAllProducts">前往购物</a>
            </h3>
        </c:when>
        <%--如果购物车为空则让它看到的是空页面--%>
        <c:otherwise>
            <table class="table table-hover table-striped table-bordered table-condensed text-center">
                ...
                <tr>
                    <td colspan="8" align="right" style="height: 60px">
                        <button class="btn btn-info margin-right-15" id="clearCartBtn" type="button"
                                onclick="showRemoveCartMore()">删除选中项
                        </button>
                        <button class="btn btn-info  margin-right-15" type="button" onclick="clearAll()"> 清空购物车
                        </button>
                        <button class="btn btn-info margin-right-15" type="button" onclick="goToShoping()"> 继续购物
                        </button>
                        <button class="btn btn-info " type="button" onclick="settleAccounts()">结算</button>
                    </td>
                </tr>
                <tr>
...
<script>
    ...
    function removeAllProduct() {
        $.ajax({
            url:'${pageContext.request.contextPath}/front/cart/clearAllProductFromCart',
            type:"POST",
    ...
    function settleAccounts() {
        ...
        if (selects.length == 0) {
            ...
            $.ajax({
                url:'${pageContext.request.contextPath}/front/cart/addTempOrderItem',
                data:{'count': count, 'price': price, 'orderCartIds': orderCartIds},
                type:"POST",
                success:function (result) {
                    if (result.code == "200") {
                        //成功，则转到确认订单页面
